...
export default function TodoList() {
  ...
  const toggleImportant = (index) => {
    ...
    const toggledTodo = { ...updatedTodos[index], important: !updatedTodos[index].important };
  ...
  const handleTaskCompletion = (index) => {
    ...
    const confirmation = window.confirm(`Congratulations! You have completed the task: ${taskCompleted}`);
    if (confirmation) {
  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center min-h-screen py-2" style={{ fontFamily: ' font-pixelify-sans' }}>
      ...
      <div className="bg-blue-300 shadow-md rounded px-8 pt-6 pb-8 mb-4">
        ...
        <ul>
          {todos.map((todo, index) => (
            <li key={index} className="flex justify-between items-center mb-2 font-pixelify-sans">
              <span className={todo.important ? 'text-red-600 font-bold  rounded focus:outline-none focus:shadow-outline mr-2' : 'text-black'}>{todo.text}</span>
              <div>
                <button
                  className={`bg-red-400 hover:bg-red-600 text-white font-bold py-1 px-2 rounded focus:outline-none focus:shadow-outline mr-2 font-pixelify-sans ${todo.important && 'bg-red-600'}`}
